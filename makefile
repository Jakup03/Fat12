...
# Znacznik czasowy: 2023-01-17 15:25:30.463436
#

OUTDIR		:= build-dir

CC          := gcc
CC_FLAGS    := -Wall -fdiagnostics-color -Wextra -Wno-error=implicit-fallthrough -Werror=vla -D_ANSI_OUTPUT -Wno-error=parentheses -DINSIDE_DANTE -pedantic -ggdb3 -xc -Werror -Wno-parentheses -std=c11 -Wno-error=unused-parameter -D_GNU_SOURCE -fmax-errors=5 
...
LD          := gcc
LD_FLAGS    := -Wl,-cref -ggdb3 -Wl,-wrap,main -Wl,-Map=main.map 
LD_LIBS     := -lm 
